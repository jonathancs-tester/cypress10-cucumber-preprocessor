...
stepDefinitions | `cypress/e2e` when `nonGlobalStepDefinitions` is true <br> `cypress/support/step_definitions` when `nonGlobalStepDefinitions` is false | Path to the folder containing our step definitions.

## How to organize the tests

### Single feature files

Put your feature files in `cypress/e2e/`

Example: cypress/e2e/Google.feature
...
Easier to show than to explain, so, assuming the feature file is in `cypress/e2e/Google.feature` , as proposed above, the preprocessor will read all the files inside `cypress/e2e/Google/`, so:
...
We also have a way to create reusable step definitions. Put them in `cypress/e2e/common/`
...
Here is an [example](cypress/support/step_definitions/customParameterTypes.js) with related [.feature file](cypress/e2e/CustomParameterTypes.feature)
...
The trick consists in adding the "env" property with the "TAGS" subproperty in the cypress.config.js configuration file. It would look something like this:
...
```shell
  ./node_modules/.bin/cypress-tags run -g 'cypress/e2e/**/*.feature'
```

or 

```shell
  ./node_modules/.bin/cypress-tags run --glob 'cypress/e2e/**/*.feature'
```
...
in your cypress.config.js to have a clean view of your tests in the cypress dashbord, and also so cypress doesn't try to run your step definition files as tests in CI.
